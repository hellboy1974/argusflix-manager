    ...
    try:
        ...
        for member in file_members:
            ...
            if not name or name.endswith("/"):
                continue
            # Normalize path separators to forward slashes for cross-platform zip compatibility
            name = name.replace("\\", "/")
...
def _gpg_run(cmd, input_data=None, timeout=30):
    ...
    if not hasattr(os, "posix_spawn"):
        ...
        try:
            ...
            # Replace cmd[0] with the resolved executable path
...
class PluginUIStaticAPIView(APIView):
    """
    Serves static files from the plugins/<plugin_key>/ui/ directory.
    Supports token authentication via standard headers or '?token=...' query parameter.
    """
    ...
    def get(self, request, key, path="index.html"):
        ...
        # 1. Try to authenticate using simplejwt if user is not already authenticated
        ...
        # 2. Assert authenticated user
        if not user or not user.is_authenticated:
            return HttpResponseForbidden("Authentication credentials were not provided or are invalid.")
            
        # 3. Assert minimum user level (STANDARD user or higher)
        if user.user_level < User.UserLevel.STANDARD:
            return HttpResponseForbidden("You do not have permission to access this resource.")
            
        # 4. Assert network access allowed
        if not network_access_allowed(request, "UI", user):
            return HttpResponseForbidden("Network access denied.")
            
        # 5. Check plugin folder and serve
        pm = PluginManager.get()
        lp = pm.get_plugin(key)
        plugin_dir = getattr(lp, "path", None)
        if not plugin_dir:
            plugin_dir = os.path.join(pm.plugins_dir, key)
            
        ui_dir = os.path.join(plugin_dir, "ui")
        if not os.path.isdir(ui_dir):
            raise Http404("Plugin UI directory not found")
            
        abs_ui_dir = os.path.abspath(ui_dir)
        return django_static_serve(request, path, document_root=abs_ui_dir)
