...
class PluginManager:
        ...
        try:
            ...
            for entry in sorted(os.listdir(self.plugins_dir)):
                ...
                if has_manifest and isinstance(manifest, dict):
                    ...
                    manifest_has_ui = bool(manifest.get("has_ui", False))
                    manifest_icon = manifest.get("icon", "") if isinstance(manifest.get("icon"), str) else ""
                    manifest_admin_only = manifest.get("admin_only", True) != False
                else:
                    manifest_has_ui = False
                    manifest_icon = ""
                    manifest_admin_only = True
                ...
                def _make_placeholder() -> LoadedPlugin:
                    return LoadedPlugin(
                        ...
                        folder_name=entry,
                        legacy=legacy,
                        has_ui=manifest_has_ui,
                        icon=manifest_icon,
                        admin_only=manifest_admin_only,
                    )
                ...
                try:
                    ...
                    if lp:
                        ...
                        lp.legacy = legacy
                        lp.has_ui = manifest_has_ui
                        lp.icon = manifest_icon
                        lp.admin_only = manifest_admin_only
    ...
    def _load_plugin(
        ...
        *,
        ...
        has_ui = getattr(instance, "has_ui", False)
        icon = getattr(instance, "icon", "")
        admin_only = getattr(instance, "admin_only", True)
            ...
            plugins.append(
                {
                    ...
                    "has_ui": getattr(lp, "has_ui", False),
                    "icon": getattr(lp, "icon", ""),
                    "admin_only": getattr(lp, "admin_only", True),
                }
            )
