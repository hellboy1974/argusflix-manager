# 🚀 Deployment- & Git-Leitfaden für deine angepasste Dispatcharr-Version

Dieses Dokument beschreibt, wie du diese angepasste Version von Dispatcharr in einem **privaten GitHub-Repository** verwaltest und auf verschiedenen Systemen (**Ubuntu Server, Synology NAS, Unraid**) mittels Docker Compose bereitstellst.
...
```text
dispatcharr_reloaded/
├── .gitignore               <-- Bereits für dich erstellt!
├── Dispatcharr-0.25.1/      <-- Der Core-Code von Dispatcharr
...
1. Öffne ein Terminal im Stammverzeichnis `dispatcharr_reloaded/`.
2. **WICHTIG:** Überprüfe, ob im Unterordner `Dispatcharr-0.25.1/` ein versteckter Ordner namens `.git/` existiert. Wenn ja, lösche ihn (`rmdir /s /q Dispatcharr-0.25.1\.git` unter Windows oder `rm -rf Dispatcharr-0.25.1/.git` unter Linux). Andernfalls wird Git diesen Ordner als "Submodul" betrachten, was das Hinzufügen von Code erschwert.
3. Initialisiere das Git-Repository im Hauptordner:
   ```bash
   git init
   ```
4. Erstelle ein neues **privates** Repository auf GitHub (z. B. `dispatcharr-custom`).
...
6. Füge die Dateien hinzu und committe sie:
   ```bash
   git add .
   git commit -m "Initial commit of custom Dispatcharr, plugins, and scripts"
   ```
...
Es gibt zwei Möglichkeiten, deine Anpassungen in Docker zu betreiben. Welches Szenario zutrifft, hängt davon ab, ob du den **Core-Code** im Ordner `Dispatcharr-0.25.1` direkt bearbeitet hast.
...
* Du nutzt das offizielle, vorgefertigte Docker-Image `ghcr.io/dispatcharr/dispatcharr:latest`.
...
### 🔹 Szenario B: Du hast den Core-Code in `Dispatcharr-0.25.1` selbst verändert
...
* Du hast hierbei zwei Möglichkeiten:
  1. **Lokal auf dem Server bauen:** In der `docker-compose.yml` nutzt du `build: ./Dispatcharr-0.25.1` anstelle von `image: ...`. Docker baut das Image dann direkt beim Start auf deinem Server. (Kann auf Synology/Unraid langsam sein).
...
1. Klone dein privates Repository auf den Server:
   ```bash
   git clone https://github.com/DEIN_BENUTZERNAME/DEIN_REPO_NAME.git /opt/dispatcharr
   cd /opt/dispatcharr
   ```
...
1. Erstelle auf deiner Synology einen Ordner unter `/volume1/docker/dispatcharr/`.
2. Kopiere deine Ordner `plugins/` und `scripts/` (z. B. via Git oder über die File Station) dorthin:
   - `/volume1/docker/dispatcharr/plugins/`
   - `/volume1/docker/dispatcharr/scripts/`
3. Öffne den **Container Manager** (DSM 7.2+) auf deiner Synology.
4. Gehe auf **Projekt** -> **Erstellen**.
5. Wähle als Pfad den erstellten Ordner `/volume1/docker/dispatcharr`.
...
* **Weg 1 (Empfohlen): Docker Compose Plugin**
  ...
  2. Klone dein Repository oder lade die Ordner in dein Appdata-Verzeichnis (typischerweise `/mnt/user/appdata/dispatcharr/`).
...
* **Weg 2: Manuelles Unraid Docker Template (Ohne Compose)**
  ...
  2. Trage folgende Werte ein:
     * **Name:** `dispatcharr`
     * **Repository:** `ghcr.io/dispatcharr/dispatcharr:latest` (oder dein eigenes Image)
     * **WebUI:** `http://[IP]:[PORT:9191]`
     * **Pfad-Mapping 1:** Container-Pfad `/data` -> Host-Pfad `/mnt/user/appdata/dispatcharr/data`
     * **Pfad-Mapping 2 (Plugins):** Container-Pfad `/data/plugins` -> Host-Pfad `/mnt/user/appdata/dispatcharr/plugins`
     * **Pfad-Mapping 3 (Scripts):** Container-Pfad `/data/scripts` -> Host-Pfad `/mnt/user/appdata/dispatcharr/scripts`
     * **Umgebungsvariable 1:** `DISPATCHARR_ENV` = `aio`
     * **Umgebungsvariable 2:** `DISPATCHARR_ALLOWED_SCRIPT_DIRS` = `/data/scripts`

---

## 🔒 4. Lizenzhinweis (AGPL-3.0)
Dispatcharr ist unter der **GNU AGPL-3.0** lizenziert. Da du das Repository **privat** hältst und für dich selbst betreibst (bzw. im privaten Kreis), ist das absolut kein Problem. Falls du das Repository jemals öffentlich (Public) machst, stelle sicher, dass du die AGPL-3.0-Lizenzbedingungen einhältst und deine Modifikationen ebenfalls unter derselben Lizenz öffentlich zugänglich machst.
