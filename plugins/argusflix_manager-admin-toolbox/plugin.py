...
logger = logging.getLogger("plugins.argusflix_manager_admin_toolbox")

class Plugin:
    name = "ArgusFlix Admin Toolbox"
    version = "1.0.0"
    description = "Die ultimative Admin-Toolbox für ArgusFlix. Vereint Stream-Checker, Senderlogo-Grabber, Logo-Cleanup, Kanalnamen-Optimierer, EPG-Healer, Kanal-Duplikate-Merger und Backup/Restore-Werkzeuge."
    author = "Antigravity"
    ...
    def _grab_logos(self, settings_dict, plugin_logger):
        ...
        logos_dir = os.environ.get("ARGUSFLIX_LOGOS_DIR", "/data/logos")
        ...
        try:
            req = urllib.request.Request(
                trees_url, 
                headers={"User-Agent": "ArgusFlix-Admin-Toolbox-Plugin/1.0.0"}
            )
    ...
    def _backup_db(self, settings_dict, plugin_logger):
        ...
        db_file = base_dir / "argusflix.db"
        
        if not db_file.exists():
            return {"status": "error", "message": "SQLite-Datenbank 'argusflix.db' wurde unter BASE_DIR nicht gefunden."}
            
        backup_dir = base_dir / "data" / "backups"
        os.makedirs(backup_dir, exist_ok=True)
        
        timestamp = timezone.now().strftime('%Y%m%d_%H%M%S')
        zip_name = f"argusflix_manager_backup_{timestamp}.zip"
        ...
        try:
            ...
            temp_db = backup_dir / f"argusflix_manager_{timestamp}.db.tmp"
            shutil.copy2(db_file, temp_db)
            
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                zipf.write(temp_db, arcname="argusflix.db")
        ...
        try:
            ...
            with zipfile.ZipFile(backup_file, 'r') as zipf:
                ...
                with open(temp_db, "wb") as f:
                    f.write(zipf.read("argusflix.db"))
