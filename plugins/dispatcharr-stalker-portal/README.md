# Dispatcharr Stalker Portals Integrator Plugin

Integrate Stalker IPTV portals (Live TV, VOD Movies, and TV Series) into Dispatcharr using portal URLs and MAC addresses. Features automatic random stream verification to only import verified active content.
...
- **Dynamic VOD Patching**: Extends Dispatcharr's built-in VOD and TV Series proxy engine at runtime to seamlessly play Stalker portal VOD movies and episodes.
...
- **Automatic Background Scheduling**: Set a 5-field standard cron schedule (e.g., `0 3 * * *` to run at 3:00 AM every night) to automatically synchronize the Stalker portals in the background. It dynamically integrates into Dispatcharr's native `django-celery-beat` scheduler.
- **Visual Highlighting of New Entries**: Automatically prefixes newly discovered and imported channels, movies, series, or episodes with a `🆕 ` emoji in their names, making them immediately recognizable in the Dispatcharr library dashboard.
...
- **Native UI Management**: Imports everything into Dispatcharr's native database models. Manage your channels, movies, and series directly using Dispatcharr's native dashboard UI (enable/disable, custom logos, metadata).

## Installation

1. Package this directory as a `.zip` file:
   ```bash
   zip -r dispatcharr-stalker-portal.zip dispatcharr-stalker-portal/
   ```
2. In the Dispatcharr UI, navigate to the **Plugins** page.
...
- **Local Stream Host**: The URL where your Dispatcharr backend is accessible by its proxy server (e.g., `http://127.0.0.1:5656` for local dev, or `http://web:9191` for Docker environment).
