...
# ----------------------------------------------------------------------
# Dispatcharr's standard VOD streaming views call get_stream_url() on the
    ...
    try:
        ...
        if len(parts) == 5:
            ...
            task_obj, task_created = PeriodicTask.objects.update_or_create(
                ...
                defaults={
                    "crontab": schedule,
                    "task": "dispatcharr_stalker_portal.sync_task",
                    "args": "[]",
                    "kwargs": "{}",
                    "enabled": True,
                }
...
# ----------------------------------------------------------------------
@shared_task(name="dispatcharr_stalker_portal.sync_task")
            ...
            for portal in portals:
                ...
                try:
                    ...
                    if sync_live_tv:
                        ...
                        # Trigger Dispatcharr Standard M3U parsing
