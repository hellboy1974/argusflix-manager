...
def migrate_vod_logos_forward(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("""
            ...
            FROM argusflix_manager_channels_logo l
            WHERE l.id IN (
...
def migrate_vod_logos_backward(apps, schema_editor):
    ...
    """
    Logo = apps.get_model('argusflix_manager_channels', 'Logo')
...
def cleanup_migrated_logos(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("""
            DELETE FROM argusflix_manager_channels_logo
            WHERE id IN (
                SELECT l.id
                FROM argusflix_manager_channels_logo l
                INNER JOIN vod_vodlogo v ON l.url = v.url
                LEFT JOIN argusflix_manager_channels_channel c ON c.logo_id = l.id
                WHERE c.id IS NULL
            )
        """)
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ...
        ('argusflix_manager_channels', '0013_alter_logo_url'),  # Ensure Logo table exists
    ]
