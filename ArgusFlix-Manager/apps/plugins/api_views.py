...
MAX_PLUGIN_IMPORT_FILES = getattr(settings, "ARGUSFLIX_PLUGIN_IMPORT_MAX_FILES", 2000)
MAX_PLUGIN_IMPORT_BYTES = getattr(settings, "ARGUSFLIX_PLUGIN_IMPORT_MAX_BYTES", 200 * 1024 * 1024)
MAX_PLUGIN_IMPORT_FILE_BYTES = getattr(settings, "ARGUSFLIX_PLUGIN_IMPORT_MAX_FILE_BYTES", 200 * 1024 * 1024)
...
OFFICIAL_KEY_PATH = os.path.join(
    os.path.dirname(__file__), "keys", "argusflix_manager-plugins.pub"
)
        ...
        for repo in repos:
            ...
            for p in repo_plugins:
                ...
                if not plugin_data.get("icon_url") and registry_url:
                    # registry_url is e.g. https://github.com/ArgusFlix/Plugins
...
class PluginInstallFromRepoAPIView(PluginAuthMixin, APIView):
    ...
    @extend_schema(
        ...
        request=inline_serializer(name="PluginInstallFromRepoRequest", fields={
            ...
            "min_argusflix_manager_version": serializers.CharField(required=False, allow_blank=True),
            "max_argusflix_manager_version": serializers.CharField(required=False, allow_blank=True),
        }),
        responses={
    ...
    def post(self, request):
        ...
        # Version compatibility check against the running ArgusFlix version
        min_version = request.data.get("min_argusflix_manager_version")
        max_version = request.data.get("max_argusflix_manager_version")
        if min_version or max_version:
            from version import __version__ as app_version
            try:
                if min_version and _compare_versions(app_version, min_version) < 0:
                    return Response(
                        {"error": f"This plugin version requires ArgusFlix {min_version} or newer (you have {app_version})"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                if max_version and _compare_versions(app_version, max_version) > 0:
                    return Response(
                        {"error": f"This plugin version requires ArgusFlix {max_version} or older (you have {app_version})"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
