...
def _get_pg_args() -> list[str]:
    ...
    return [
        ...
        "-d", db_config.get("NAME", "argusflix_manager"),
    ]
...
def create_backup() -> Path:
    ...
    backup_name = f"argusflix_manager-backup-{timestamp}.zip"
    backup_file = backup_dir / backup_name

    logger.info(f"Creating backup: {backup_name}")

    with tempfile.TemporaryDirectory(prefix="argusflix_manager-backup-") as temp_dir:
        ...
        with ZipFile(backup_file, "w", compression=ZIP_DEFLATED, allowZip64=True) as zip_file:
            ...
            # Add metadata
            metadata = {
                "format": "argusflix_manager-backup",
                "version": 2,
...
def restore_backup(backup_file: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="argusflix_manager-restore-") as temp_dir:
        temp_path = Path(temp_dir)
...
def list_backups() -> list[dict]:
    ...
    for backup_file in sorted(backup_dir.glob("argusflix_manager-backup-*.zip"), reverse=True):
