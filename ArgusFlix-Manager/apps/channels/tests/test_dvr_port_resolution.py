...
class DVRStreamBaseURLTests(SimpleTestCase):
    ...
    @patch.dict(os.environ, {'ARGUSFLIX_ENV': 'aio'}, clear=True)
    def test_aio_explicit_uses_localhost_5656(self):
        """Explicit ARGUSFLIX_ENV=aio also uses loopback port 5656."""
        url = get_dvr_stream_base_url()
        self.assertEqual(url, 'http://127.0.0.1:5656')

    @patch.dict(os.environ, {'ARGUSFLIX_ENV': 'dev'}, clear=True)
    def test_dev_mode_uses_localhost_5656(self):
        """Dev mode shares the container with uwsgi — uses loopback port 5656."""
        url = get_dvr_stream_base_url()
        self.assertEqual(url, 'http://127.0.0.1:5656')

    @patch.dict(os.environ, {'ARGUSFLIX_ENV': 'modular', 'ARGUSFLIX_PORT': '9191'}, clear=True)
    ...
    @patch.dict(os.environ, {'ARGUSFLIX_ENV': 'modular', 'ARGUSFLIX_PORT': '8080'}, clear=True)
    def test_modular_custom_port(self):
        """Modular mode respects ARGUSFLIX_PORT."""
        url = get_dvr_stream_base_url()
        self.assertEqual(url, 'http://web:8080')

    @patch.dict(os.environ, {
        'ARGUSFLIX_ENV': 'modular',
        'ARGUSFLIX_PORT': '9191',
        'ARGUSFLIX_WEB_HOST': 'argusflix_manager_web',
    }, clear=True)
    def test_modular_custom_web_host(self):
        """ARGUSFLIX_WEB_HOST overrides the default 'web' service name."""
        url = get_dvr_stream_base_url()
        self.assertEqual(url, 'http://argusflix_manager_web:9191')

    @patch.dict(os.environ, {
        'ARGUSFLIX_INTERNAL_TS_BASE_URL': 'http://custom:1234',
        'ARGUSFLIX_ENV': 'modular',
    }, clear=True)
    def test_explicit_override_always_wins(self):
        """ARGUSFLIX_INTERNAL_TS_BASE_URL takes priority over all other settings."""
        url = get_dvr_stream_base_url()
    ...
    def test_explicit_override_strips_trailing_slash(self):
        """Trailing slash is stripped from ARGUSFLIX_INTERNAL_TS_BASE_URL."""
        url = get_dvr_stream_base_url()
        self.assertEqual(url, 'http://custom:1234')
