...
class LogoViewSet(viewsets.ModelViewSet):
    ...
    def upload(self, request):
        ...
        # Validate file
        try:
            from argusflix_manager.utils import validate_logo_file
...
def _stop_dvr_clients(channel_uuid, recording_id=None):
    ...
    If recording_id is None, all ArgusFlix-DVR clients for the channel are stopped
    ...
    for raw_id in client_ids:
        try:
            ...
            if not (ua_s and "ArgusFlix-DVR" in ua_s):
                continue
            # When a recording_id is specified, only stop the client for that recording.
            # Each run_recording task connects with User-Agent "ArgusFlix-DVR/recording-{id}",
