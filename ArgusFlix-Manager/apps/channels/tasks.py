...
def _dvr_build_ffmpeg_cmd(stream_url, recording_id, hls_m3u8, hls_seg_pattern, hls_start_number):
    ...
    return [
        ...
        "-user_agent", f"ArgusFlix-DVR/recording-{recording_id}",
...
def get_dvr_stream_base_url():
    ...
    Priority:
    1. ARGUSFLIX_INTERNAL_TS_BASE_URL — explicit override, always wins.
    2. Modular mode (ARGUSFLIX_ENV=modular) — celery runs in a separate container
       and must reach the web container by its Docker service name on ARGUSFLIX_PORT.
       Override the host with ARGUSFLIX_WEB_HOST for non-standard compose setups.
    3. AIO / dev / debug — celery shares the container with uwsgi which binds on
       port 5656; use 127.0.0.1 to avoid any nginx layer.
    """
    explicit = os.environ.get('ARGUSFLIX_INTERNAL_TS_BASE_URL')
    if explicit:
        return explicit.rstrip('/')

    argusflix_manager_env = os.environ.get('ARGUSFLIX_ENV', 'aio').lower()

    if argusflix_manager_env == 'modular':
        host = os.environ.get('ARGUSFLIX_WEB_HOST', 'web')
        port = os.environ.get('ARGUSFLIX_PORT', '9191')
