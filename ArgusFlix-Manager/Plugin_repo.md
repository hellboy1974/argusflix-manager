# ArgusFlix Plugin Repository Specification

How to create and host a plugin repository that ArgusFlix can consume.
...
ArgusFlix discovers plugins from remote repositories using a two-level manifest system:
...
Users add a repo by its manifest URL. ArgusFlix fetches and caches the repo manifest periodically (default: every 6 hours, configurable). The UI displays all plugins from enabled repos in a browsable store.

---

## Repo Manifest

The repo manifest is the entry point. ArgusFlix fetches this URL and caches the response.
    ...
    "plugins": [
      {
        ...
        "license": "MIT",
        "deprecated": false,
        "repo_url": "https://github.com/acmelabs/argusflix_manager-weather",
        ...
        "min_argusflix_manager_version": "2.5.0",
        "max_argusflix_manager_version": null
      }
    ]
  },
...
ArgusFlix accepts two top-level shapes:
...
`registry_name` is required. ArgusFlix rejects repos that are missing it.

Third-party repos must not use names that could be confused with an official ArgusFlix repo. The following words are blocked in `registry_name` (case-insensitive):

- "official"
- "argusflix_manager plugins"
- "argusflix_manager repo"
- "argusflix_manager official"
...
| --------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `registry_name` | **Yes**  | Display name for the repo. Must not contain words like "official" or "argusflix_manager" that could be mistaken for an official repo (see [Name Restrictions](#name-restrictions)). |
...
| `latest_md5`              | No       | MD5 checksum of the latest release zip. Informational only - not validated by ArgusFlix.                                                  |
...
| `min_argusflix_manager_version` | No       | Minimum ArgusFlix version required. Install is blocked if the running version is older.                                                   |
| `max_argusflix_manager_version` | No       | Maximum ArgusFlix version supported. Install is blocked if the running version is newer.                                                  |
...
**Icon fallback:** If `icon_url` is missing and `registry_url` is set, ArgusFlix generates a fallback URL by converting the GitHub URL to a raw content URL:
...
```json
{
  "slug": "weather_display",
  "name": "Weather Display",
  "description": "Shows weather information on the ArgusFlix dashboard",
  "author": "Acme Labs",
  "license": "MIT",
  "latest_version": "1.2.5",
  "registry_name": "Acme Labs Plugins",
  "registry_url": "https://github.com/acmelabs/argusflix_manager-plugins",
  "versions": [
    {
      "version": "1.2.5",
    ...
    },
    {
      ...
      "min_argusflix_manager_version": "2.4.0"
    }
  ],
  "latest": {
    "version": "1.2.5",
...
| `min_argusflix_manager_version` | No       | Minimum compatible ArgusFlix version.                                                                                                                                                                                                                                                                                                                                                                                            |
| `max_argusflix_manager_version` | No       | Maximum compatible ArgusFlix version.                                                                                                                                                                                                                                                                                                                                                                                            |
...
- A single version entry built from `latest_version`, `latest_url`, `latest_sha256`, `min_argusflix_manager_version`, `max_argusflix_manager_version`, and `last_updated`
...
Signing your repo manifest lets ArgusFlix verify it hasn't been tampered with. Signing is **optional** - unsigned repos work fine but show an "unverified" badge in the UI.

### How It Works

1. You generate a GPG keypair.
2. You sign the manifest JSON and include the detached signature in the response.
3. When adding the repo in ArgusFlix, the user pastes your public key.
4. ArgusFlix verifies the signature on every manifest fetch.
...
When a user adds your repo URL, they can paste your public key. ArgusFlix stores the key per-repo and uses it for verification. Users can update the key at any time from the repo management UI.

If you don't provide a key and the repo is not the official ArgusFlix repo, signature verification is skipped (result: `null`).
...
1. **Version compatibility check** - if `min_argusflix_manager_version` or `max_argusflix_manager_version` is set, the running ArgusFlix version is compared. Install is blocked if out of range.
...
ArgusFlix detects updates by comparing `installed_version` (stored in the database) against `latest_version` from the repo manifest. This uses repo-level fields only - per-plugin manifests are not needed for update detection.
...
Any web server that serves JSON works. ArgusFlix fetches manifests server-side, so CORS is not needed.
...
- [ ] Include `min_argusflix_manager_version` where applicable
...
- [ ] Set `max_argusflix_manager_version` if a plugin is incompatible with newer releases
    ...
    "plugins": [
      {
        ...
        "min_argusflix_manager_version": "string (semver)",
        "max_argusflix_manager_version": "string (semver) or null"
      }
    ]
  },
...
```json
{
  ...
  "latest": {
    ...
    "min_argusflix_manager_version": "string",
    "max_argusflix_manager_version": "string or null"
  }
}
```
