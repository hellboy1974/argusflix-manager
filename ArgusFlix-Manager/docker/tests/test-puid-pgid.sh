...
#     and the ArgusFlix release image for upgrade tests)
...
#
# Options:
#   --skip-build    Skip Docker image build (use existing argusflix_manager:puid-test image)
...
# Configuration
###############################################################################
IMAGE_NAME="argusflix_manager:puid-test"
RELEASE_IMAGE="ghcr.io/argusflix_manager/argusflix_manager:latest"
BASE_IMAGE="ghcr.io/argusflix_manager/argusflix_manager:base"
...
check_pg_accessible() {
    local container="$1" os_user="$2" db="${3:-argusflix_manager}" port="${4:-5432}"
...
setup_old_pg_data_manual() {
    ...
    docker run --rm --entrypoint bash -v "${volume}:/data" "$IMAGE_NAME" -c '
        ...
        su - postgres -c "createdb -p 5432 --encoding=UTF8 --owner=dispatch argusflix_manager"
...
test_modular_mode() {
    ...
    docker run -d --name "$pg_name" --network "$net" \
        ...
        -e POSTGRES_DB=argusflix_manager \
...
test_custom_port() {
    ...
    if wait_for_ready "$name"; then
        ...
        if docker exec "$name" su - dispatch -c \
            "psql -d argusflix_manager -p 5433 -tAc 'SELECT 1;'" 2>/dev/null | grep -q 1; then
...
test_pg_major_upgrade() {
    ...
    # This simulates an older ArgusFlix installation that used PG 16.
    ...
    if ! docker run --rm -v "${vol}:/data" --entrypoint bash postgres:16 -c '
        ...
        gosu postgres createdb -p 5432 --encoding=UTF8 --owner=dispatch argusflix_manager
...
test_pg_upgrade_post_puid() {
    ...
    if ! docker run --rm -v "${vol}:/data" --entrypoint bash "$IMAGE_NAME" -c '
        ...
        su - dispatch -c "psql -U dispatch -d template1 -p 5432 -c \"CREATE DATABASE argusflix_manager OWNER dispatch ENCODING '\''UTF8'\'';\""
