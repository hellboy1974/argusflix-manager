...
fi  # End of ARGUSFLIX_ENV != modular check
...
check_external_postgres_version() {
    ...
    # PG_VERSION is from ArgusFlixBase
    ...
    if [[ "$EXTERNAL_VERSION" -lt "$MIN_REQUIRED_VERSION" ]]; then
        ...
        echo ""
        echo "  Your external PostgreSQL database is too old for ArgusFlix."
        ...
        echo "   PostgreSQL version should be compatible with ArgusFlix."
    fi
