...
if ! [[ "$ARGUSFLIX_PORT" =~ ^[0-9]+$ ]]; then
    echo "⚠️  Warning: ARGUSFLIX_PORT is not a valid integer, using default port 9191"
    ARGUSFLIX_PORT=9191
fi
sed -i "s/NGINX_PORT/${ARGUSFLIX_PORT}/g" /etc/nginx/sites-enabled/default
