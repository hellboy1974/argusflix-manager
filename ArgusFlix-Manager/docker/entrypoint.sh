...
export POSTGRES_DB=${POSTGRES_DB:-argusflix_manager}
...
if [[ "${ARGUSFLIX_ENV:-}" == "modular" && "${POSTGRES_SSL:-}" == "true" ]]; then
    export POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-}"
else
    export POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-secret}"
fi
export ARGUSFLIX_ENV=${ARGUSFLIX_ENV:-aio}
if [[ "$ARGUSFLIX_ENV" == "aio" ]]; then
...
export ARGUSFLIX_PORT=${ARGUSFLIX_PORT:-9191}
...
export ARGUSFLIX_VERSION=$(python -c "import sys; sys.path.append('/app'); import version; print(version.__version__)")
export ARGUSFLIX_TIMESTAMP=$(python -c "import sys; sys.path.append('/app'); import version; print(version.__timestamp__ or '')")

# Display version information with timestamp if available
if [ -n "$ARGUSFLIX_TIMESTAMP" ]; then
    echo "📦 ArgusFlix version: ${ARGUSFLIX_VERSION} (build: ${ARGUSFLIX_TIMESTAMP})"
else
    echo "📦 ArgusFlix version: ${ARGUSFLIX_VERSION}"
fi
export ARGUSFLIX_LOG_LEVEL
# Set log level with default if not provided
ARGUSFLIX_LOG_LEVEL=${ARGUSFLIX_LOG_LEVEL:-INFO}
# Convert to uppercase
ARGUSFLIX_LOG_LEVEL=${ARGUSFLIX_LOG_LEVEL^^}


echo "Environment ARGUSFLIX_LOG_LEVEL set to: '${ARGUSFLIX_LOG_LEVEL}'"

# Also make the log level available in /etc/environment for all login shells
#grep -q "ARGUSFLIX_LOG_LEVEL" /etc/environment || echo "ARGUSFLIX_LOG_LEVEL=${ARGUSFLIX_LOG_LEVEL}" >> /etc/environment

# Translate ArgusFlix POSTGRES_SSL_* env vars into libpq-recognized PGSSL*
...
variables=(
    ...
    ARGUSFLIX_ENV ARGUSFLIX_DEBUG ARGUSFLIX_LOG_LEVEL ARGUSFLIX_ENABLE_IP_LOOKUP
    REDIS_HOST REDIS_PORT REDIS_DB REDIS_PASSWORD REDIS_USER POSTGRES_DIR ARGUSFLIX_PORT
    ARGUSFLIX_VERSION ARGUSFLIX_TIMESTAMP LIBVA_DRIVERS_PATH LIBVA_DRIVER_NAME LD_LIBRARY_PATH
...
> /etc/profile.d/argusflix_manager.sh
...
for var in "${variables[@]}"; do
    ...
    if [ -n "${!var+x}" ]; then
        ...
        echo "export ${var}='${!var}'" >> /etc/profile.d/argusflix_manager.sh
...
chmod +x /etc/profile.d/argusflix_manager.sh

# Ensure root's .bashrc sources the profile.d scripts for interactive non-login shells
if ! grep -q "profile.d/argusflix_manager.sh" /root/.bashrc 2>/dev/null; then
    cat >> /root/.bashrc << 'EOF'

# Source ArgusFlix environment variables
if [ -f /etc/profile.d/argusflix_manager.sh ]; then
    . /etc/profile.d/argusflix_manager.sh
fi
EOF
fi
...
if [ "${POSTGRES_SSL_KEY:-}" = "$FIXED_KEY_PATH" ]; then
    ...
    sed -i "s|export POSTGRES_SSL_KEY=.*|export POSTGRES_SSL_KEY='$FIXED_KEY_PATH'|" /etc/profile.d/argusflix_manager.sh
fi
...
. /app/docker/init/03-init-argusflix_manager.sh

# Start PostgreSQL if NOT in modular mode (using external database)
if [[ "$ARGUSFLIX_ENV" != "modular" ]]; then
...
if [ ${#pids[@]} -gt 0 ]; then
    echo "⏳ ArgusFlix is running. Monitoring processes..."
    set +e
