...
cd /app
source /argusflix_managerpy/bin/activate
...
celery -A argusflix_manager beat -l info &
...
nice -n "$NICE_LEVEL" celery -A argusflix_manager worker -Q dvr -n dvr@%h --pool=threads --concurrency=20 -l info &
# Default prefork worker: every queue except `dvr`.
nice -n "$NICE_LEVEL" celery -A argusflix_manager worker -Q celery -n default@%h --autoscale=6,1 -l info
