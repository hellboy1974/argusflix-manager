...
const AvailablePluginCard = ({
  ...
  const meetsMinVersion =
    !plugin.min_argusflix_manager_version ||
    compareVersions(appVersion, plugin.min_argusflix_manager_version) >= 0;
  const meetsMaxVersion =
    !plugin.max_argusflix_manager_version ||
    compareVersions(appVersion, plugin.max_argusflix_manager_version) <= 0;
            ...
            ? [
                {
                  ...
                  min_argusflix_manager_version: plugin.min_argusflix_manager_version,
                  max_argusflix_manager_version: plugin.max_argusflix_manager_version,
    ...
    >
      ...
      <Group justify="space-between" mt="sm" align="center" wrap="nowrap">
        {!meetsVersion &&
          (() => {
            const parts = [];
            if (!meetsMinVersion)
              parts.push(`${plugin.min_argusflix_manager_version} or newer`);
            if (!meetsMaxVersion)
              parts.push(`${plugin.max_argusflix_manager_version} or older`);
            const label = !meetsMinVersion
              ? `Min ${plugin.min_argusflix_manager_version}`
              : `Max ${plugin.max_argusflix_manager_version}`;
            return (
              <Tooltip
                label={`Incompatible: requires ArgusFlix ${parts.join(' and ')} (you have v${appVersion})`}
              >
      ...
      >
        <Stack gap="md">
          ...
          <Text size="sm" c="dimmed">
            ...
            stop working with future versions of ArgusFlix. It is recommended
            to look for an alternative.
          </Text>
          <PluginSecurityWarning>
            Plugins run server-side code with full access to your ArgusFlix
          ...
          >
            <Stack gap="md">
              ...
              <PluginSecurityWarning>
                ...
                ArgusFlix instance and its data. Only install plugins from
      ...
      >
        <Stack gap="md">
          ...
          <Text size="sm">
            A restart of ArgusFlix may be required to fully unload the plugin.
          </Text>
      ...
      >
        <Stack gap="md">
          ...
          <Text size="sm">
            A restart of ArgusFlix may be required for the plugin to be fully
            loaded.
          </Text>
          {pluginIsDisabled && (
            <>
