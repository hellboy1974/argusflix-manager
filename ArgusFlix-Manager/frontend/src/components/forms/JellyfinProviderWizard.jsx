...
export default function JellyfinProviderWizard({ opened, onClose, server, onSave }) {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      api_token: (val) => (val.trim() ? null : 'Jellyfin Token erforderlich'),
    }
  });
  ...
  const handleSave = async (values) => {
    try {
      ...
      if (server) {
        ...
        notifications.show({ title: 'Erfolg', message: 'Jellyfin Server hinzugefügt.', color: 'green' });
      }
      onSave();
    } catch (err) {
  ...
  return (
    <Modal 
      opened={opened} 
      onClose={onClose} 
      title={<Text weight={600} size="lg">{server ? 'Jellyfin Server Bearbeiten' : 'Jellyfin Server Hinzufügen'}</Text>}
      size="xl"
    >
            ...
            <Tabs.Panel value="overview">
              <Group grow align="flex-start">
                <Stack>
                  ...
                  <TextInput label="Server Name" placeholder="z.B. Mein Jellyfin" {...form.getInputProps('name')} />
                  <TextInput label="Server URL" placeholder="http://192.168.1.100:8096" {...form.getInputProps('base_url')} />
                  <PasswordInput label="Jellyfin Token" placeholder="Dein Token" {...form.getInputProps('api_token')} />
                ...
                <Stack>
                  {server ? (
                    ...
                    <Box mt="xl">
                       <Text c="dimmed" size="sm">Speichere den Jellyfin Server, um weitere Optionen (Test, Sync, Kategorien) zu erhalten.</Text>
                    </Box>
                  )}
                </Stack>
              </Group>
