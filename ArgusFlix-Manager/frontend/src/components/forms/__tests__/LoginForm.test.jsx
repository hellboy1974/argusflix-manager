...
describe('LoginForm', () => {
  ...
  describe('remember me', () => {
    it('saves username to localStorage when Remember Me is checked', async () => {
      ...
      await waitFor(() => {
        expect(localStorage.getItem('argusflix_manager_remembered_username')).toBe(
          'savedUser'
        );
      });
    });
    ...
    it('removes username from localStorage when Remember Me is unchecked', async () => {
      localStorage.setItem('argusflix_manager_remembered_username', 'oldUser');
      setupMocks({ loginResult: Promise.resolve(true) });
      renderLoginForm();
      ...
      await waitFor(() => {
        expect(
          localStorage.getItem('argusflix_manager_remembered_username')
        ).toBeNull();
      });
    });

    it('pre-fills username from localStorage on mount', () => {
      localStorage.setItem('argusflix_manager_remembered_username', 'storedUser');
      setupMocks();
      renderLoginForm();
  ...
  describe('save password', () => {
    it('saves encoded password to localStorage when Save Password is checked', async () => {
      ...
      await waitFor(() => {
        expect(
          localStorage.getItem('argusflix_manager_saved_password')
        ).not.toBeNull();
      });
    });

    it('removes password from localStorage when Save Password is unchecked', async () => {
      localStorage.setItem('argusflix_manager_saved_password', btoa('oldPass'));
      setupMocks({ loginResult: Promise.resolve(true) });
      renderLoginForm();
      ...
      await waitFor(() => {
        expect(localStorage.getItem('argusflix_manager_saved_password')).toBeNull();
      });
    });

    it('pre-fills password from localStorage on mount', () => {
      localStorage.setItem('argusflix_manager_remembered_username', 'storedUser');
      localStorage.setItem('argusflix_manager_saved_password', btoa('storedPass'));
      setupMocks();
      renderLoginForm();
