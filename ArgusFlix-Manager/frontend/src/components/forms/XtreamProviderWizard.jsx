...
export default function XtreamProviderWizard({ opened, onClose, portal, onSave }) {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      username: (val) => (val.trim() ? null : 'Benutzername erforderlich'),
      password: (val) => (val.trim() ? null : 'Passwort erforderlich'),
    }
  });
  ...
  const handleSave = async (values) => {
    try {
      ...
      if (portal) {
        ...
        notifications.show({ title: 'Erfolg', message: 'Panel hinzugefügt.', color: 'green' });
      }
      onSave();
    } catch (err) {
  ...
  return (
    <Modal 
      opened={opened} 
      onClose={onClose} 
      title={<Text weight={600} size="lg">{portal ? 'Xtream Panel Bearbeiten' : 'Xtream Panel Hinzufügen'}</Text>}
      size="xl"
    >
          ...
          <Box mt="md" mih={350}>
            <Tabs.Panel value="overview">
              <Group grow align="flex-start">
                <Stack>
                  <Title order={5}>Panel Informationen</Title>
                  <TextInput label="Profilname" placeholder="z.B. Mein Provider" {...form.getInputProps('name')} />
                  <TextInput label="Server URL" placeholder="http://domain.com:8080" {...form.getInputProps('server_url')} />
                  <TextInput label="Benutzername" placeholder="Dein Username" {...form.getInputProps('username')} />
                  <PasswordInput label="Passwort" placeholder="Dein Passwort" {...form.getInputProps('password')} />
                  
                  <Switch 
                    label="Verbindung Aktiv"
                    description="Wenn deaktiviert, wird das Panel nicht mehr synchronisiert oder angezeigt."
                ...
                <Stack>
                  {portal ? (
                    ...
                    <Box mt="xl">
                       <Text c="dimmed" size="sm">Speichere das Panel, um weitere Optionen zur Synchronisation zu erhalten.</Text>
                    </Box>
                  )}
                </Stack>
              </Group>
