...
export default function PlexProviderWizard({ opened, onClose, server, onSave }) {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      api_token: (val) => (val.trim() ? null : 'Plex Token erforderlich'),
    }
  });
  ...
  const handleSave = async (values) => {
    try {
      ...
      if (server) {
        ...
        notifications.show({ title: 'Erfolg', message: 'Plex Server gespeichert.', color: 'green' });
      } else {
        await api.post('/api/v1/mediaservers/', payload);
        notifications.show({ title: 'Erfolg', message: 'Plex Server hinzugefügt.', color: 'green' });
      }
      onSave();
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (!server) return;
    if (window.confirm('Möchtest Du diesen Plex Server wirklich löschen?')) {
      try {
        setLoading(true);
  ...
  return (
    <Modal 
      opened={opened} 
      onClose={onClose} 
      title={<Text weight={600} size="lg">{server ? 'Plex Server Bearbeiten' : 'Plex Server Hinzufügen'}</Text>}
      size="xl"
    >
            ...
            <Tabs.Panel value="overview">
              <Group grow align="flex-start">
                <Stack>
                  ...
                  <TextInput label="Server Name" placeholder="z.B. Mein Plex" {...form.getInputProps('name')} />
                  <TextInput label="Server URL" placeholder="http://192.168.1.100:32400" {...form.getInputProps('base_url')} />
                  <PasswordInput label="Plex Token" placeholder="X-Plex-Token" {...form.getInputProps('api_token')} />
                ...
                <Stack>
                  {server ? (
                    ...
                    <Box mt="xl">
                       <Text c="dimmed" size="sm">Speichere den Plex Server, um weitere Optionen (Test, Sync, Kategorien) zu erhalten.</Text>
                    </Box>
                  )}
                </Stack>
              </Group>
