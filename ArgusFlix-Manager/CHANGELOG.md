...
- **VOD basic sync now stores additional metadata from the provider's stream list.** When a provider includes `director`, `cast`, `release_date`, or `trailer`/`youtube_trailer` in its `get_vod_streams` response, ArgusFlix now captures and stores those fields in `custom_properties` during the initial sync pass. Previously, this data was discarded and only populated if an advanced per-movie refresh was triggered. - Thanks [@nemesbak](https://github.com/nemesbak)
...
- **`debian_install.sh` switched from Gunicorn to uWSGI with gevent workers.** The Debian/LXC bare-metal installer now deploys ArgusFlix under the same uWSGI + gevent stack used by the Docker image, eliminating a class of compatibility differences between the two deployment paths. The installer writes a `uwsgi-debian.ini` next to the app and manages uWSGI via a systemd service. The nginx site config now uses `uwsgi_pass` + `include uwsgi_params` instead of `proxy_pass`, which correctly populates `SERVER_PORT` in the WSGI environ so M3U playlist URLs include the configured port number (fixing the port-stripping bug from #1267 for bare-metal installs). Python 3.13 is now provisioned through uv's managed runtime so the install works on Debian 12 and Ubuntu 24.04 LTS regardless of the system Python version.
- **`get_vod_streams` XC API response was missing metadata fields available from basic sync.** When a provider includes `director`, `cast`, `release_date`, `plot`, `genre`, or `year` in its `get_vod_streams` response, ArgusFlix stores those fields during the basic sync pass but was not including them in its own `get_vod_streams` XC output. The endpoint now outputs all six fields. The `trailer` key in the response also mapped to the wrong internal key (`trailer` instead of `youtube_trailer`) following the storage key rename, so the trailer field was always empty until an advanced refresh ran. Both issues are corrected. Users with existing libraries should trigger a VOD provider refresh to populate the missing fields. (Fixes #1228)
...
- **Official plugin repository manifest URL moved to GitHub Pages.** The default `OFFICIAL_REPO_URL` now points to `https://argusflix_manager.github.io/Plugins/manifest.json` instead of the raw GitHub content URL. GitHub Pages avoids opaque caching on raw content that could serve stale manifests without indication. A data migration updates existing `PluginRepo` rows marked `is_official=True` in place; fresh installs pick up the new URL from the model default after the seed migration runs. — Thanks [@sethwv](https://github.com/sethwv)
...
- **Browser-local web player output profile setting.** A new "Web Player Output Profile" select in Settings → UI Settings lets each browser choose which output profile (if any) is applied when previewing streams in the built-in floating player. The preference is saved to `localStorage` (`argusflix_manager-player-prefs`) and picked up by every preview URL builder at click time, so a user whose account default transcodes to AC3 for their media server can still get a browser-compatible audio stream from the preview player without changing their account settings.
...
- **`get_client_ip()` in `argusflix_manager/utils.py` cleaned up.** Removed dead `.split(',')[0]` call and misleading variable name left over from a copy-paste of the `X-Forwarded-For` pattern. `X-Real-IP` is always a single IP (set by nginx from `$remote_addr`), so no splitting is needed. Behavior is unchanged.
...
- `ARGUSFLIX_WEB_HOST` environment variable for modular deployments. The Celery container needs to reach the uWSGI / web container to fetch HLS segments while recording (FFmpeg pulls from the public stream URL, the same way any other client does). `get_dvr_stream_base_url()` resolves the base URL deterministically: AIO / dev / debug containers reach uWSGI on `127.0.0.1:$ARGUSFLIX_PORT` since Celery and uWSGI share the container, while modular deployments use `http://$ARGUSFLIX_WEB_HOST:$ARGUSFLIX_PORT` and default `ARGUSFLIX_WEB_HOST` to `web` (the compose service name). Documented as a commented-out override in `docker/docker-compose.yml`.
...
- **HDHomeRun SSDP advertiser**. The `apps/hdhr/ssdp.py` module and the `start_ssdp()` call in `apps.hdhr.apps.HdhrConfig.ready()` have been removed. The implementation advertised a `urn:schemas-upnp-org:device:MediaServer:1` UPnP MediaServer device on `udp/1900`, but ArgusFlix does not implement the UPnP MediaServer Content Directory Service that such an advertisement promises, and the `LOCATION`-targeted `/hdhr/device.xml` returns HDHomeRun-flavored XML rather than the UPnP descriptor a generic UPnP browser expects. None of the major HDHomeRun clients (Plex, Emby, Jellyfin, Channels DVR, Kodi) use SSDP for tuner discovery, they use SiliconDust's native UDP broadcast on `udp/65001`, which ArgusFlix does not currently implement. The advertiser ran a listener thread, a broadcaster thread, a bound multicast socket, and emitted a `NOTIFY` broadcast every 30 seconds for no consumer. Tuners can still be added in any HDHomeRun-aware client by entering the ArgusFlix URL manually (e.g. `http://<host>:9191/`).
...
- **Plugin Hub**: administrators can now browse, install, and update plugins directly from remote repositories via a new Plugin Hub page in Settings. (Closes #393) — Thanks [@sethwv](https://github.com/sethwv)
  ...
  - Update managed plugins when a newer version is available from their source repo. Version compatibility constraints (`min_argusflix_manager_version` / `max_argusflix_manager_version`) are enforced at install time.
  ...
  - Add plugin repositories by manifest URL. The official ArgusFlix Plugins repository is pre-configured; third-party repos are supported by supplying an optional GPG public key.
...
- Donate button added to the sidebar footer. A heart icon links to the project's Open Collective page, visible in both expanded and collapsed states. Hovering shows a "Support ArgusFlix" tooltip. The version string is also now clickable to copy it to the clipboard.
...
- Docker base image cleanup: removed `python-is-python3`, `python3-pip`, and `streamlink` from the apt package list in `ArgusFlixBase`. `python3-pip` and `streamlink` were pulling outdated system Python packages (e.g. `requests 2.31.0`, `cryptography 41.0.7`, `lxml 5.2.1`) into the system Python's site-packages despite the app running entirely in the uv-managed venv at `/argusflix_managerpy`. `streamlink` is already installed in the venv via `pyproject.toml`. `python-is-python3` is unnecessary as `PATH` resolves bare `python` to the venv binary.
...
- Floating video player improvements
  ...
  - **Persistent state**: Size, position, volume level, and mute state are now saved across sessions using a single `argusflix_manager-player-prefs` localStorage key. Size and position are restored on next open (clamped to the current viewport); volume and mute are restored when the player initialises.
...
- DVR bug fixes — Thanks [@CodeBormen](https://github.com/CodeBormen)
  ...
  - **DVR client isolation**: Stop and Cancel operations now identify the target client by recording ID (via `User-Agent: ArgusFlix-DVR/recording-{id}`), ensuring only the correct proxy client is torn down and never affecting other recordings on the same channel.
...
- Modular mode deployment hardening — Thanks [@CodeBormen](https://github.com/CodeBormen)
  ...
  - **DVR recording broken in modular mode**: Internal TS stream URL candidates hardcoded port `9191`, so recordings failed when `ARGUSFLIX_PORT` was set to any other value. URL construction now reads `ARGUSFLIX_PORT` from the environment via the new `build_dvr_candidates()` helper. `ARGUSFLIX_PORT` is also now explicitly passed to the Celery container in `docker-compose.yml`.
  ...
  - **Redis wait stripping environment variables**: The modular-mode Redis readiness check ran as a uWSGI `exec-pre` hook, which executes under `su -` and strips Docker environment variables, making `ARGUSFLIX_ENV` and `REDIS_HOST` unavailable. Moved to the container entrypoint so all env vars are present.
  - **Stale environment variables after container restart**: `/etc/profile.d/argusflix_manager.sh` was only written on the first container run; restarts with changed env vars (e.g. a rotated `POSTGRES_PASSWORD`) retained stale values. The file is now truncated and rewritten on every startup. `/etc/environment` entries are likewise updated rather than skipped when a key already exists. All exported values are now quoted to prevent breakage from special characters.
...
- Event-driven webhooks and script execution (Integrations): Added new Integrations feature that enables event-driven execution of custom scripts and webhooks in response to system events. (Closes #203)
  ...
  - **Event data delivery**: available as environment variables in scripts (prefixed with `ARGUSFLIX_`), POST payloads for webhooks, and plugin execution payloads
...
- Plugin stop hooks: ArgusFlix now calls a plugin's optional `stop()` method (or `run("stop")` action) when disabling, deleting, or reloading plugins to allow graceful shutdown.
...
- App initialization efficiency: Refactored app initialization to prevent redundant execution across multiple worker processes. Created `argusflix_manager.app_initialization` utility module with `should_skip_initialization()` function that prevents custom initialization tasks (backup scheduler sync, developer notifications sync) from running during management commands, in worker processes, or in development servers. This significantly reduces startup overhead in multi-worker deployments (e.g., uWSGI with 10 workers now syncs the scheduler once instead of 10 times). Applied to both `CoreConfig` and `BackupsConfig` apps.
...
- Fixed modular Docker Compose deployment and entrypoint/init scripts to properly support `ARGUSFLIX_ENV=modular`, use external PostgreSQL/Redis services, and handle port, version, and encoding validation (Closes #324, Fixes #61, #445, #731) - Thanks [@CodeBormen](https://github.com/CodeBormen)
...
- Docker init script now validates ARGUSFLIX_PORT is an integer before using it, preventing sed errors when Kubernetes sets it to a service URL like `tcp://10.98.37.10:80`. Falls back to default port 9191 when invalid (Fixes #737)
...
- LXC unable to access daemons launched by uWSGI ([#575](https://github.com/ArgusFlix/ArgusFlix/issues/575), [#576](https://github.com/ArgusFlix/ArgusFlix/issues/576), [#577](https://github.com/ArgusFlix/ArgusFlix/issues/577))
...
- Time Zone Settings added to the application ([#482](https://github.com/ArgusFlix/ArgusFlix/issues/482), [#347](https://github.com/ArgusFlix/ArgusFlix/issues/347))
...
- Channel Creation Improvements:
  - Ability to specify channel number during channel creation ([#377](https://github.com/ArgusFlix/ArgusFlix/issues/377), [#169](https://github.com/ArgusFlix/ArgusFlix/issues/169))
...
- **Plugin System:**
  - Extensible Plugin Framework - Developers can build custom functionality without modifying ArgusFlix core
...
- [#350](https://github.com/ArgusFlix/ArgusFlix/issues/350): Allow DVR recordings to be played via the UI
- [#349](https://github.com/ArgusFlix/ArgusFlix/issues/349): DVR screen doesn't populate consistently
- [#340](https://github.com/ArgusFlix/ArgusFlix/issues/340): Global find and replace
- [#311](https://github.com/ArgusFlix/ArgusFlix/issues/311): Stat's "Current Speed" does not reflect "Buffering Speed" setting
- [#304](https://github.com/ArgusFlix/ArgusFlix/issues/304): Name ignored when uploading logo
- [#300](https://github.com/ArgusFlix/ArgusFlix/issues/300): Updating Logo throws error
- [#286](https://github.com/ArgusFlix/ArgusFlix/issues/286): 2 Value/Column EPG in Channel Edit
- [#280](https://github.com/ArgusFlix/ArgusFlix/issues/280): Add general text field in M3U/XS profiles
- [#190](https://github.com/ArgusFlix/ArgusFlix/issues/190): Show which stream is being used and allow it to be altered in channel properties
- [#155](https://github.com/ArgusFlix/ArgusFlix/issues/155): Additional column with EPG assignment information / Allow filtering by EPG assignment
- [#138](https://github.com/ArgusFlix/ArgusFlix/issues/138): Bulk Channel Edit Functions
...
- Optimized logo fetching with proper timeouts and headers ([#101](https://github.com/ArgusFlix/ArgusFlix/issues/101), [#217](https://github.com/ArgusFlix/ArgusFlix/issues/217))
...
- **Infrastructure & Performance:**
  ...
  - New environment variable to set logging level: `ARGUSFLIX_LOG_LEVEL` (default: `INFO`, available: `TRACE`, `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`)
