...
This document explains how to build, install, and use Python plugins in ArgusFlix. It covers discovery, the plugin interface, settings, actions, how to access application APIs, and examples.
...
- Override with env var: `ARGUSFLIX_PLUGINS_DIR`.
...
`plugin.json` lets ArgusFlix list your plugin safely without executing code. It should live next to `plugin.py`.
...
- `stop(context)` (optional callable): Invoked when the plugin is disabled, deleted, or reloaded so you can gracefully shut down any processes you started. If `stop()` is not defined but you have an action with id `stop`, ArgusFlix will call `run("stop", {}, context)` as a fallback.
...
## Accessing ArgusFlix APIs from Plugins
...
ArgusFlix plugins run **inside** the ArgusFlix backend process. That means they already have direct access to the app’s models, tasks, and internal utilities.  
Plugins **should not** ask users for “ArgusFlix URL”, “Admin Username”, or “Admin Password” just to call the API. That is unnecessary and unsafe because:
...
In rare cases you may need to call a ArgusFlix HTTP endpoint (for example, to reuse an existing API response serializer). In that case:
...
- ArgusFlix will not import or execute plugin code unless the plugin is enabled.
