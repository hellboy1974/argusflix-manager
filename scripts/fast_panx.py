...
Dosyab="/sdcard/"  +panel.replace(":","_").replace('/','') +"@🅵🅰🆂🆃🆇PAN.txt"
def yax(hits):
...
├⭕"""+SINGENC+"""
├⭕Exp ➤ """+str(trh)+"""
├⭕Type/Scan date ➤ """+uzmanm+"""/"""+str(time.strftime("%Y:%m:%d"))+"""
├⭕Image ➤ """+str(durum)+"""
├⭕Vpn ➤ """+str(vpn)+""""""+str(playerapi)+"""
╰⭕m3uLink ➤ """+str(m3ulink)+"""
"""
		if len(kanalsayisi) > 1:
			imza=imza+"""╭─⭕ Channels ➤"""+str(kanalsayisi)+"""
├⭕Vod ➤"""+str(filmsayisi)+"""
...
def echok(mac,bot,total,hitc,oran,tokenr):
	...
	#global cpmx
		...
		echo=("""
╭─●\33[33m FastpanX      
├● \33[0m\33[1;107;44m maclink  \33[1;7m """+str(panel)+""" \33[0m     
...
def hitprint(mac,trh):
	#playsound('/sdcard/sounds/ching.mp3')
	file = pathlib.Path()
	try:
		if file.exists():
		    ad.mediaPlay()
...
├⭕🌍Real ➤ http://"""+realm+""":"""+port+""""""+str(uzmanm2)+""""""+str(buri)+"""
├⭕📡Port ➤ """+port+"""
├⭕👩‍User ➤ """+userm+"""
├⭕🔑Pass ➤ """+pasm+"""
├⭕📆Exp ➤ """+bitism+""" 
